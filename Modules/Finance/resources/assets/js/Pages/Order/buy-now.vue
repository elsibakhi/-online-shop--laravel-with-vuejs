...
import { buyNowFormBalance,getStripeCheckoutPage } from '@/actions/Modules/Finance/Http/Controllers/OrderController';
  ...
  <Card class="max-w-xl p-4 mx-auto mt-10 space-y-6">
    ...
    <CardContent>
      ...
      <div class="card">
        <Tabs value="wallet">
            <TabList >
              <Tab  value="direct">{{ t('purchase.direct_bid') }}</Tab>
              <Tab  value="wallet">{{ t('purchase.wallet_bid') }}</Tab>
            ...
            <TabPanels>
              <TabPanel value="direct">
                ...
                <a target="_blank" :href="getStripeCheckoutPage().url+`?item_id=${item.data.id}&quantity=${form.quantity}`">
